...
agents/.env (carregado por run.py antes deste módulo):
  ...
  VITUAL_SUPABASE_REST — 1 activa REST /rest/v1; 0 ou omitido = só PostgresTools + DATABASE_URL
  VITUAL_SUPABASE_STORAGE — 0 desliga Storage mesmo com bucket definido (opcional)
  VITUAL_SUPABASE_STORAGE_BUCKET — id do bucket (ex. vitual-relatorios); se definido, activa upload Markdown
  VITUAL_SUPABASE_STORAGE_PREFIX — opcional (default relatorios); pasta lógica dentro do bucket
  VITUAL_SUPABASE_STORAGE_USE_SERVICE_ROLE — 1 força service role só para Storage; senão segue VITUAL_SUPABASE_USE_SERVICE_ROLE
  VITUAL_SUPABASE_STORAGE_PUBLIC_READ — 1 inclui URL pública na resposta (só faz sentido com bucket público)
  VITUAL_LOCAL_PDF — 1 força PDF local; 0 desliga; omitido desliga PDF se VITUAL_SUPABASE_STORAGE_BUCKET estiver definido e Storage não estiver desligado com VITUAL_SUPABASE_STORAGE=0
  VITUAL_SUPABASE_USE_SERVICE_ROLE — recomendado 1 para uploads Storage com SERVICE_ROLE_KEY
  ...
  VITUAL_AGENTOS_FROM_HUB — 1 (default) regista um Agent Agno por linha em hub_agente_identidade (id = agente_slug); 0 só ``chat``.
  VITUAL_AGENTOS_TENANT_ID ou DEFAULT_TENANT_ID — filtra tenant_id (uuid).
  VITUAL_AGENTOS_SLUGS — subconjunto: slugs separados por vírgula.
  VITUAL_AGENTOS_PLAYBOOK_MAX_BYTES — limite ao descarregar playbook_public_url (default 500000).
  VITUAL_AGENTOS_LEGACY_CHAT — 1 (default) se houver agentes Hub, acrescenta também id=``chat`` com o mesmo prompt que o primeiro agente Hub (atalho); 0 só slugs CRM.
